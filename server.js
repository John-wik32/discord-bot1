...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    input[type="text"], input[type="date"], input[type="time"], input[type="password"], select { width: 100%; padding: 12px; border: 2px solid #e0e0e0; border-radius: 6px; font-size: 14px; font-family: inherit; }
    input:focus, select:focus { outline: none; border-color: #667eea; box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1); }
    .file-upload { border: 2px dashed #667eea; border-radius: 6px; padding: 30px; text-align: center; cursor: pointer; background: #f8f9ff; }
    ...
    .btn-send:hover:not(:disabled) { background: #5568d3; transform: translateY(-2px); }
    ...
    .message { padding: 15px; border-radius: 6px; margin-bottom: 20px; display: none; font-size: 14px; }
    ...
    .schedule-title { font-weight: 600; color: #333; }
    .schedule-time { color: #666; font-size: 12px; margin-top: 4px; }
    .schedule-files { margin-top: 10px; padding: 8px; background: white; border-radius: 4px; font-size: 12px; }
    .schedule-file { padding: 4px 0; }
    ...
    .edit-fields { display: none; margin-top: 12px; padding-top: 12px; }
    ...
    <div id="create" class="page active">
      <div class="form-section">
        ...
        <form id="mediaForm">
          ...
          <div class="form-group"><label>Upload Media Files</label><div class="file-upload" id="fileUpload"><div>Click or drag files here</div><div style="color: #999; font-size: 12px; margin-top: 5px;">PNG, JPG, GIF, MP4, MOV</div><input type="file" id="fileInput" multiple accept="image/*,video/*"></div><div class="file-list" id="fileList"></div></div>
...
async function loadChannels() {
  ...
  currentPassword = pwd;
  if (!pwd) { showMessage('Please enter password', 'info'); return; }
  try {
    ...
    select.innerHTML = '<option value="">Select a channel...</option>' + channels.map(ch => '<option value="' + ch.id + '">' + ch.guild + ' - ' + ch.name + '</option>').join('');
...
document.getElementById('sendNowBtn').addEventListener('click', async (e) => {
  ...
  if (!pwd || !channelId || selectedFiles.length === 0) { showMessage('Fill all fields', 'error'); return; }
  try {
    document.getElementById('sendNowBtn').disabled = true;
    showMessage('Uploading...', 'info');
    const formData = new FormData();
    selectedFiles.forEach(file => formData.append('files', file));
    const uploadRes = await fetch(API_URL + '/api/upload', { method: 'POST', headers: { 'x-dashboard-pwd': pwd }, body: formData });
  ...
  finally { document.getElementById('sendNowBtn').disabled = false; }
});

let scheduleSecondClick = false;
document.getElementById('scheduleBtn').addEventListener('click', async (e) => {
  if (scheduleSecondClick && !isScheduling) {
    scheduleSecondClick = false;
    e.preventDefault();
    const pwd = document.getElementById('password').value;
    const channelId = document.getElementById('channel').value;
    const title = document.getElementById('title').value;
    ...
    if (!pwd || !channelId || !date || !time || selectedFiles.length === 0) { showMessage('Fill all fields', 'error'); return; }
    try {
...
async function saveSchedule(id) {
  ...
  try {
    const res = await fetch(API_URL + '/api/schedule/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'x-dashboard-pwd': currentPassword }, body: JSON.stringify({ title: newTitle }) });
    if (!res.ok) throw new Error('Failed');
    ...
    showSchedulesMessage('✓ Updated!', 'success');
  } catch (err) { showSchedulesMessage('Error: ' + err.message, 'error'); }
}
...
async function deleteSchedule(id) {
  ...
  try {
    const res = await fetch(API_URL + '/api/schedule/' + id, { method: 'DELETE', headers: { 'x-dashboard-pwd': currentPassword } });
    if (!res.ok) throw new Error('Failed');
    allSchedules = allSchedules.filter(s => s.id !== id);
    renderSchedules();
    showSchedulesMessage('✓ Deleted!', 'success');
  } catch (err) { showSchedulesMessage('Error: ' + err.message, 'error'); }
}
